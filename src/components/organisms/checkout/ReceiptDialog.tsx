...
export default function ReceiptDialog({ visible, transaction, onClose }: ReceiptDialogProps) {
  ...
  const header = (
    <div className="flex align-items-center gap-3">
      <div
        ...
        style={{ width: '48px', height: '48px', borderRadius: '12px', backgroundColor: 'var(--pos-success)' }}
      >
  ...
  return (
    <Dialog
      ...
      modal
      closable={false}
      contentStyle={{ backgroundColor: 'var(--pos-bg-secondary)', padding: '0' }}
      headerStyle={{ backgroundColor: 'var(--pos-bg-secondary)', borderBottom: '1px solid var(--pos-border)', padding: '1.5rem' }}
    >
      <div className="p-4" style={{ backgroundColor: 'white', color: '#1f2937' }}>
        <div className="text-center mb-4">
          <h3 className="m-0 text-lg font-bold">SuperMarket POS</h3>
          <p className="m-0 text-sm" style={{ color: '#6b7280' }}>Av 5 Barrio la Cabrera</p>
          <p className="m-0 text-sm" style={{ color: '#6b7280' }}>Tel: (311) 3581407</p>
        </div>
      ...
      <div className="flex gap-3 p-4">
        ...
        <Button
          label="Nueva Venta"
          className="flex-1"
          style={{ height: '50px', borderRadius: '12px', background: 'linear-gradient(135deg, var(--pos-accent) 0%, #1d4ed8 100%)', border: 'none' }}
          onClick={onClose}
        />
      </div>
    </Dialog>
